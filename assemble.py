...
class Matrices:
    ...
    def assemble_matrices(self, triangulation, alpha):
        ...
        Me_batch = (areas / 12).unsqueeze(1).unsqueeze(2) * torch.eye(3, device=self.device, dtype=self.dtype).unsqueeze(
            0) * 2  # Shape (N, 3, 3)
        ...
        Ke_batch = torch.zeros((vertices.shape[0], 3, 3), device=self.device, dtype=self.dtype)
        for i in range(3):
            for j in range(3):
