...
class CourtemancheRamirezNattel:
    def __init__(self, dt: float, device: torch.device, dtype: torch.dtype = torch.float64):
        ...
        self.Ki = torch.full((1,), self.Ki_init).to(device)
        self.d = torch.full((1,), self.d_init).to(device)
        self.f = torch.full((1,), self.f_init).to(device)
        self.f_Ca = torch.full((1,), self.f_Ca_init).to(device)
        self.h = torch.full((1,), self.h_init).to(device)
        self.j = torch.full((1,), self.j_init).to(device)
        self.m = torch.full((1,), self.m_init).to(device)
        self.oa = torch.full((1,), self.oa_init).to(device)
        self.oi = torch.full((1,), self.oi_init).to(device)
        self.u = torch.full((1,), self.u_init).to(device)
        self.ua = torch.full((1,), self.ua_init).to(device)
        self.ui = torch.full((1,), self.ui_init).to(device)
        self.v = torch.full((1,), self.v_init).to(device)
        self.w = torch.full((1,), self.w_init).to(device)
        self.xr = torch.full((1,), self.xr_init).to(device)
        self.xs = torch.full((1,), self.xs_init).to(device)
    ...
    def initialize(self, n_nodes: int):
        ...
        self.Ca_rel = torch.full((n_nodes,), self.Ca_rel_init).to(self.device)
        self.Ca_up = torch.full((n_nodes,), self.Ca_up_init).to(self.device)
        self.Cai = torch.full((n_nodes,), self.Cai_init).to(self.device)
        self.Ki = torch.full((n_nodes,), self.Ki_init).to(self.device)
        self.d = torch.full((n_nodes,), self.d_init).to(self.device)
        self.f = torch.full((n_nodes,), self.f_init).to(self.device)
        self.f_Ca = torch.full((n_nodes,), self.f_Ca_init).to(self.device)
        ...
        self.oa = torch.full((n_nodes,), self.oa_init).to(self.device)
        self.oi = torch.full((n_nodes,), self.oi_init).to(self.device)
        self.u = torch.full((n_nodes,), self.u_init).to(self.device)
        self.ua = torch.full((n_nodes,), self.ua_init).to(self.device)
        self.ui = torch.full((n_nodes,), self.ui_init).to(self.device)
        self.v = torch.full((n_nodes,), self.v_init).to(self.device)
        self.w = torch.full((n_nodes,), self.w_init).to(self.device)
        self.xr = torch.full((n_nodes,), self.xr_init).to(self.device)
