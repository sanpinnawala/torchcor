...
class Courtemanche:
    ...
    def construct_tables(self):
        ...
        Cai_tab = torch.zeros((Cai.shape[0], self.Cai_ti.NROWS)).to(self.device)
        ...
        V_tab = torch.zeros((V.shape[0], self.V_ti.NROWS)).to(self.device)
        ...
        fn_tab = torch.zeros((fn.shape[0], self.fn_ti.NROWS)).to(self.device)
    ...
    def differentiate(self, V):
        ...
        lower_indices = torch.floor(indices).long()
        upper_indices = lower_indices + 1
        fn1 = Cai_min + lower_indices * Cai_res
        fn2 = Cai_min + upper_indices * Cai_res
