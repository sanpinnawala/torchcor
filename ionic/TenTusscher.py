...
class TenTusscher:
    ...
    def computes_rates(self, states, constants):
        ...
        algebraic[:, 17] = custom_piecewise([:, less(states[:, 0] , -40.0000), 0.0570000*torch.exp(-(states[:, 0]+80.0000)/6.80000) , True, 0.00000])
        algebraic[:, 30] = custom_piecewise([:, less(states[:, 0] , -40.0000), 2.70000*torch.exp(0.0790000*states[:, 0])+310000.*torch.exp(0.348500*states[:, 0]) , True, 0.770000/(0.130000*(1.00000+torch.exp((states[:, 0]+10.6600)/-11.1000)))])
        ...
        algebraic[:, 18] = custom_piecewise([:, less(states[:, 0] , -40.0000), (((-25428.0*torch.exp(0.244400*states[:, 0])-6.94800e-06*torch.exp(-0.0439100*states[:, 0]))*(states[:, 0]+37.7800))/1.00000)/(1.00000+torch.exp(0.311000*(states[:, 0]+79.2300))) , True, 0.00000])
        algebraic[:, 31] = custom_piecewise([:, less(states[:, 0] , -40.0000), (0.0242400*torch.exp(-0.0105200*states[:, 0]))/(1.00000+torch.exp(-0.137800*(states[:, 0]+40.1400))) , True, (0.600000*torch.exp(0.0570000*states[:, 0]))/(1.00000+torch.exp(-0.100000*(states[:, 0]+32.0000)))])
